...
public class ProfileCommand implements Command {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        if (profileDAO.existsByUsersConnect(profileDAO.getUsersConnectDAO().getByDiscordName(event.getInteraction().getUser().getName()))) {
            ...
            embedSender.send(eb);
            event.reply(CommandConstants.PROFILE_SUCCESS_MESSAGE).queue();
        } else {
            event.reply(CommandConstants.PROFILE_UNSUCCESSFUL_MESSAGE).queue();
        }
    }

    @Override
    public void execute(JsonNode data) {
        UsersConnect user = usersConnectDAO.getByTelegramName(data.findValue(CommandConstants.NAME_FROM_TELEGRAM).asText());
        if (profileDAO.existsByUsersConnect(profileDAO.getUsersConnectDAO().getByTelegramName(data.findValue(CommandConstants.NAME_FROM_TELEGRAM).asText()))) {
            EmbedBuilder eb = new EmbedBuilder()
                    .setAuthor(data.findValue(CommandConstants.NAME_FROM_TELEGRAM).asText())
                    .setTitle(data.findValue(CommandConstants.NAME_FROM_TELEGRAM).asText())
            ...
            EmbedBuilder eb = new EmbedBuilder()
                    .setAuthor(data.findValue(CommandConstants.NAME_FROM_TELEGRAM).asText())
                    .setDescription(CommandConstants.PROFILE_UNSUCCESSFUL_MESSAGE);
            embedSender.send(eb);
        }
    }
